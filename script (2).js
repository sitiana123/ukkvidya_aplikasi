...
function log() {
  // Mengambil nilai input dan mengonversinya menjadi angka
  const value = parseFloat(screen.value.trim());

  // Memeriksa apakah nilai input valid dan lebih besar dari 0
  if (isNaN(value) || value <= 0) {
    screen.value = "Error";  // Tampilkan pesan error jika input tidak valid
    return;  // Hentikan eksekusi lebih lanjut
  }

  // Membuat ekspresi untuk riwayat kalkulator
  const expression = `log(${value})`;

  // Menghitung logaritma dasar 10 dari nilai
  const result = Math.log10(value);

  // Menambahkan ekspresi dan hasil perhitungan ke riwayat kalkulator
  addToHistory(expression, result);
...
// Fungsi kalkulator lainnya di sini (seperti evaluateExpression, backspc, etc.)
